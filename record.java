...
public class record extends JFrame implements ActionListener{
	...
	//定義錄音格式
	AudioFormat af = null;
	//定義目標數據行,可以從中讀取音頻頻數據,该 TargetDataLine 接口提供從目標數據行的緩衝區讀取所捕或數據的方法
	TargetDataLine td = null;
	//定義數據行,源數據行是可以寫入數據的數據行。當混頻器的源。應用程序將音頻字節寫入源數據行，處理字節緩衝並傳遞給混頻器。
	...
	public void capture()
	{
		try {
			...
			//打開具有指定格式的行，這樣可使行獲得所有所需的系统資源並變的可操作。
			td.open(af);
			//允許某一數據行執行數據 I/O
			td.start();
